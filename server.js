...
function updateComment(url, request){
  ...
  if (!id || !requestComment) {
    ...
    savedComment.body = requestComment.body || savedComment.body.comment;
...
function deleteComment(url, request) {
  ...
  if (savedComment != null && id) {
    const user = database.users[savedComment.username];
    const article = database.articles[savedComment.articleId];

    if(user.commentIds.indexOf(id) != -1) {
      let indexOfCommentinUser = user.commentIds.indexOf(id);
      user.commentIds.splice(indexOfCommentinUser, 1);
    }

    let indexOfCommentinArticle = article.commentIds.indexOf(id);
    article.commentIds.splice(indexOfCommentinArticle, 1);
